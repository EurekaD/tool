...
from Resources.local_connection import local_connection
import yt_dlp
...
class MinIOUtil:
    ...
    def videoImgUpload(self, url, bucket_name, key):
        ...
        try:
            ...
            if isExist == "false":
                ...
                # 上传图片
                self.minioClient.put_object(bucket_name, "image/" + img_name, img_data, img_size,
    ...
    def imgUpload_update(self, img_name, url, bucket_name):
        """
        :param img_name: 图片名称
        :param url: 图片详细url路径
        :param bucket_name: 上传minio 桶名称
        :return: 返回上传公司服务器的图片路径
        :desc 更新图片路径，修改为通/爬虫名称/日期名称/照片名称
        """
